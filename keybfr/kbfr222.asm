...
KEYBFR	JP	START
	...
	DB	'KEYBFR - French AZERTY keyboard driver installer for BIOS rel. 2.22',0DH,0AH
	DB	'$'

OFF64K	EQU	0EA00H		;Offset to 64k CP/M BIOS

BIOSVER	EQU	20H		;BIOS version to check
...
MBANNER	DB	1AH,07H,16H
	...
	DB	'BIOS r2.22Fr (c)''84 MM/JBO',15H,0EH,0DH,0AH,0AH

MDRIVEM	EQU	$		;0DH,'>>> Memory Drive M: ',16H,'ENABLED',16H,0DH,0AH,0AH,00H

DKIDWN7	EQU	04A4H		;shift/ctrl/clear/special keys (logical state)
...
PATCH2D	EQU	0EDDFH-OFF64K	;insert call to intl handler in KBDSCAN
...
KBDINT2	EQU	0EDF8H-OFF64K	;skip CAPS check
KEYSYMB	EQU	0EE02H-OFF64K	;digits and symbols
KBDEXTB	EQU	0EE1BH-OFF64K	;exit with char in B
KBDEXIT	EQU	0EE1CH-OFF64K	;exit with char in A
...
PATCH3D	EQU	0EEACH-OFF64K	;CAPS flag and symbols conversion table
